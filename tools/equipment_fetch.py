...
def fetch_from_api(tier: str) -> dict | None:
    ...
    try:
        ...
        if results:
            # Grab the URL of the first valid result
            url = results[0].get("href", "")
            if not url:
                return None
                
            log.debug(f"DuckDuckGo found URL for {tier}: {url}")
            ...
            # Use LLM to extract factual data from the manual
            log.info(f"Extracting technical specs from datasheet via LLM...")
            ...
            log.info(f"Successfully extracted real-world specs from {url}")
            return parsed_data
